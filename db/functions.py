...
async def get_claim_time(db_pool: async_sessionmaker, phone_number: int) -> int:
    async with db_pool() as db_session:
        query = select(NextTimes.claim).where(NextTimes.account__ID == phone_number).where(
            NextTimes.botName == "PocketFiBot")
        result = await db_session.execute(query)
...
async def set_claim_time(db_pool: async_sessionmaker, phone_number: int, timestamp: int) -> None:
    async with db_pool() as db_session:
        query = select(NextTimes).where(NextTimes.account__ID == phone_number).where(NextTimes.botName == "PocketFiBot")
        result = await db_session.execute(query)
        next_time = result.scalars().one_or_none()

        if next_time:
            next_time.claim = timestamp
            await db_session.commit()
